...
5. **Versioning (C14)**:
    ...
    - **Rules**: Bump Minor (add/amend), Major (remove), or Patch (typos). Update Document History in the target file (global or workspace).

## Rule Tier Routing

When processing an add/amend/remove request, determine the target tier:

- **Workspace tier** → `.design/{workspace}/RULES.md`: rule references workspace-specific concerns (a named workspace's scope, tools, or patterns).
- **Global tier** → `.design/RULES.md`: rule applies to all workspaces, or no workspace is active.
...
graph TD
    ...
    D --> E[Guard: DUP across both tiers & CONSTRUCT]
...
| :--- | :--- | :--- |
| **Add** | Find highest C{N} (global) or WC{N} (workspace) → Assign next ID → Append to target tier. | Minor |
...
1. **Impact Analysis & Sync**: After write, if `TASKS.md` exists:
    ...
    - **Audit**: If rule is critical (L1/C1-C10), suggest `magic.spec audit` for compliance.
