...
> **Disambiguation**: a single unquoted word that matches both a workspace name and a focus keyword → workspace takes priority. Force focus interpretation by quoting: `/magic.analyze "engine"`.
...
Identify tech stack via config files (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `pom.xml`, etc.). Build a high-level map using directory listing (depth 2 default; extend to depth 3 only for monorepo root directories with nested `packages/` or `apps/`).
**Gitignore Filtering**: apply Invariant 7 filters *before* building the map so build artifacts and ignored dependencies do not leak into architecture inference or coverage check.
...
Source code may contain design rationale in structured comments not captured in formal specs. The Rationale Extraction system scans for these patterns and identifies "Shadow Logic" — design decisions documented only in code.
...
**Usage**: `node .magic/scripts/executor.js extract-rationale --json` for structured output. Comments matched in Python (`# MARKER:`), JS/TS/Go/Rust/C (`// MARKER:`), Shell, and PowerShell files.
...
### Shared Pre-flight (Modes A & B)

Both first-time analysis (A) and re-analysis (B) start with the same pre-flight gate:
...
3. **Gap Report** (enhanced with Confidence Taxonomy):
   ...
   - **Logic Evolution**: if code structure/logic inside covered directories has structurally drifted (e.g., >30% new sub-modules — defined as new directories or files containing logic exports; or API schema shift) → **Propose Reality Sync**: generate a structured diff or a "New Draft" version of the specification reflecting actual codebase implementation. If approved: dispatch via `spec.md` Amendment Rule (Stable → RFC). C12 cascade applies to all L2 dependents of the affected spec.
...
2. **Design Registry Audit**:
   ...
   - **Phantom Commands** (`PHANTOM_COMMAND`): the converse check. Scan the shipped engine and documentation text — `.magic/`, `docs/`, `workflows/`, `rules/`, `README.md` — for `/magic.{cmd}` command mentions and assert that each resolves to `workflows/magic.{cmd}.md`. A mention that does not → finding `PHANTOM_COMMAND {file}:{line}: /magic.{cmd} has no wrapper` (advisory). A token counts as a command mention only when it stands as its own word — preceded by whitespace, a backtick or an opening quote — so file paths such as `rules/magic.md` are never read as commands; `{cmd}` is a lowercase word (a hyphen ends the token, so `/magic.run`-executable reads as `/magic.run`); developer-facing `magic.dev.*` names are exempt. Internal modules (`pause.md` and its siblings) have no wrapper, so they must not be advertised to the user as commands. Governed by `l2-workflow-wrappers.md` §6.1.
3. **Structural Integrity** (when workspace is specified):
   ...
   - **Link Integrity**: scan `.design/`, `.magic/`, `workflows/`, `skills/`, and `rules/` files for broken relative links (404 targets). Exclude `.magic/templates/` from resolution — its paths are placeholders that resolve only once a template is instantiated into a user project (SH-3, `l1-scan-input-hygiene.md`); the exclusion is by file role, not by whether a target string looks like a placeholder. Templates stay in scope for every non-resolving structural check.
...
4. **Coverage Check**: scan project directories *within the active workspace scope (C15)*. Identify folders with NO corresponding spec file (Gap Report).
   ...
   - **Enhanced Coverage (Confidence Taxonomy)**: run `node .magic/scripts/executor.js analyze-coverage --json`. Sub-classify all files by confidence level (EXTRACTED/INFERRED/AMBIGUOUS/UNCOVERED). Include confidence breakdown in the Coverage Check report section. Flag AMBIGUOUS files for human review.
...
9. **Workspace Boundary Analysis**: run `node .magic/scripts/executor.js detect-communities --include-md`.
...
14. **Report**: consolidated list of errors, warnings, and suggested repairs.
15. **Advisory**: generate Advisory Report (see §Advisory Report) for the audited scope.
...
2. **Targeted Scan**: instead of full project scan, narrow to directories/files relevant to the focus:
   ...
   - No matches found → **HALT**: *"Could not map focus '{text}' to any project area. Try narrowing with a workspace: `/magic.analyze {workspace} \"{text}\"`, or rephrase the focus."*
