...
7. **Architectural Logic**:
   ...
   - **Guided Planning Filter**: when a planning directive is present (Guided / Scoped + Guided modes), restrict C6 selection to `Stable` specs whose filename, title, or content matches the directive (same matching approach as `analyze.md` Mode D Focus Directive). Non-matching `Stable` specs are left untouched in their current `PLAN.md`/Backlog placement — the directive narrows this invocation's scope, it does not reclassify specs it doesn't match. **Bootstrap interaction**: when the C6 Bootstrap Exception (below) activates, the filter applies identically to the Draft-spec pull — only MVC-passing Draft specs matching the directive are planned with `[Bootstrap]`; non-matching Draft specs stay untouched. Directive matches zero specs (Stable, or Draft when Bootstrap is eligible) → **HALT**: *"Could not map planning directive '{text}' to any registered spec. Run `/magic.task {workspace}` for full planning, or author it first with `/magic.spec`."*
...
1. **Pre-flight**: `node .magic/scripts/executor.js check-prerequisites --json --require-specs --verify-headers --workspace={active-workspace}`.
   ...
   - **Design-Debt Backlog** (`DESIGN_DEBT_PENDING`, per SC-2.4 of `l1-session-continuity.md`): if `check-prerequisites` reports this warning type — plan complete (no active phase in `TASKS.md`) with ≥1 open item in `PLAN.md`'s `## Backlog` — **HALT**. Report: *"Backlog holds {N} item(s) needing design input — plan has no active phase to absorb them. Run `/magic.spec {ws}` to author the queued specs, then re-run `/magic.task`."* This is the sanctioned door for `/magic.spec` at plan-complete: every other mechanical gate above is clean by construction here, so without this check nothing would ever HALT and the persisted `Next Action` (SC-2.1(c)) would keep recommending `/magic.task` itself — the command that just produced this exact HALT.
