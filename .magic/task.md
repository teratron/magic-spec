...
8. **Sync (Update Mode)**:
   ...
   - **Phantom Parent Guard**: L2 spec's parent missing from disk or `INDEX.md` (cross-workspace or local) → **HALT**. Report: *"Parent Spec `{parent-file}` (L1) is missing. Cannot plan dependent `{file}` (L2)."* Move L2 to `## Backlog` with reason: *"Missing L1 Parent (Phantom)."*
