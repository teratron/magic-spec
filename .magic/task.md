...
7. **Architectural Logic**:
   ...
   - **Guided Planning Filter**: when a planning directive is present (Guided / Scoped + Guided modes), restrict C6 selection to `Stable` specs whose filename, title, or content matches the directive (same matching approach as `analyze.md` Mode D Focus Directive). Non-matching `Stable` specs are left untouched in their current `PLAN.md`/Backlog placement — the directive narrows this invocation's scope, it does not reclassify specs it doesn't match. Directive matches zero specs → **HALT**: *"Could not map planning directive '{text}' to any registered spec. Run `/magic.task {workspace}` for full planning, or author it first with `/magic.spec`."*
