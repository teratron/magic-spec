---
description: Interactive tutorial to learn the Magic SDD lifecycle.
---

# Onboard Workflow

**Triggers:** *"Start the interactive tutorial"*, *"Onboard me"*, *"Learn SDD"*, *"Start onboarding"*
**Scope:** Interactive tutorial to guide new developers through their first Magic SDD cycle.

> **Full implementation:** `.magic/onboard.md`
> Read that file before proceeding. Do not execute any steps until it is read.
