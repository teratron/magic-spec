...
graph TD
    ...
    TASK --> CODE["🚀 Код"]
    TASK -.->|"авто: фаза завершена"| RETRO["🔍 Retrospective<br/>retrospective.md"]
...
|---|---|---|---|
| 1 | **Specification** | `specification.md` | 📋 Преобразует сырые мысли в структурированные спецификации. Проверяет их соответствие реальному проекту. Статусы: Draft → RFC → Stable → Deprecated |
| 2 | **Plan** | `plan.md` | 🗺️ Читает Stable-спеки, строит граф зависимостей, извлекает критический путь, создаёт фазовый `PLAN.md` |
| 3 | **Task** | `task.md` | ⚡ Разбивает План на атомарные задачи с треками. Режимы: Sequential и Parallel. Автоматически запускает Retrospective по завершении фазы и плана |

### Вспомогательный Воркфлоу
...
| **Check Specs** | `specification.md` | ⚖️ Пред-проверка целостности. Запускается перед созданием плана или задач, чтобы убедиться, что спеки соответствуют реальным путям, структуре и конфигам проекта |
| **Retrospective** | `retrospective.md` | 🔍 Вызывается автоматически из `task.md`: Level 1 — снапшот после каждой фазы, Level 2 — полный анализ при завершении всего плана. Не является пользовательской командой |

## 🏗️ Архитектура и Структура Директорий

Система SDD состоит из трёх основных директорий:

1. **`.agent/workflows/magic.*.md`** — Точки входа для AI-агентов (например, slash-команды в Cursor или Claude). Тонкие обёртки (~12 строк каждая), запускающие основные воркфлоу Magic. Всего **4 обёртки**: specification, plan, task, rule.
...
│   ├── magic.plan.md               #    → запускает .magic/plan.md
│   ├── magic.rule.md               #    → запускает .magic/rule.md
│   ├── magic.specification.md      #    → запускает .magic/specification.md
...
│   ├── retrospective.md        #    Движок самоанализа (вызывается автоматически из task.md)
│   ├── rule.md                 #    Воркфлоу управления конституцией (вспомогательный)
│   ├── specification.md        #    Воркфлоу авторинга спецификаций + шаблоны
│   ├── task.md                 #    Воркфлоу декомпозиции и выполнения задач
│   └── scripts/                #    Скрипты инициализации (авто-запуск)
│       ├── init.sh             #    macOS / Linux
│       └── init.ps1            #    Windows
│
...
Ретроспектива — это **механизм самосовершенствования** Magic, встроенный непосредственно в воркфлоу Task. Она замыкает петлю обратной связи, анализируя реальные данные использования SDD и генерируя практические рекомендации. Никакой ручной команды не требуется.
