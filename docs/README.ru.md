...
4. **Самосовершенствование:** Движок автоматически запускает ретроспективу для оптимизации собственной работы.
5. **Обязательная Симуляция:** Любое изменение движка проверяется через `magic.simulate` перед деплоем.

## 🛠️ Требования

Установщик `magic-spec` мультиплатформенный и требует наличия одной из следующих технологий:

- **Node.js (>= 16)** — для установки через `npx`
- **Python (>= 3.8)** и [**uv**](https://github.com/astral-sh/uv) — для установки через `uvx`
...
| **Simulate** | `simulate.md` | 🧪 Логическая симуляция работы движка для поиска «шероховатостей» без написания реального кода. |
...
1. **`.agent/workflows/magic.*.md`** — Точки входа для AI-агентов (например, slash-команды в Cursor или Claude). Тонкие обёртки (~12 строк каждая), запускающие основные воркфлоу Magic. Всего **6 обёрток**: onboard, specification, plan, task, run, rule.
...
│   ├── magic.rule.md               #    → конституция проекта
│   ├── magic.run.md                #    → выполнение плана
│   ├── magic.simulate.md           #    → симуляция логики движка
│   ├── magic.spec.md               #    → спецификации и Explore mode
│   └── magic.task.md               #    → планирование и генерация задач
│
