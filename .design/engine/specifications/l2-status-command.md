...
- [l2-session-checkpoint.md](l2-session-checkpoint.md) - Owns the shared resume predicate that the in-flight branch of §5.3 calls.
...
- **Strictly read-only**: no writes to `.design/`, `.magic/`, or any project file; no finalize invocation; no runtime version bump. "No writes" means no artifact writes: a script the briefing calls may record a non-fatal finding to the diagnostics sink (`.design/.cache/` — runtime state, not an artifact; [l1-engine-diagnostics.md](l1-engine-diagnostics.md) §2, "read-only commands emit but never deliver"). (Authoring the command files is itself a one-time engine change governed by C14 during implementation.)
