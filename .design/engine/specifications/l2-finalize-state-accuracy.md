...
| Path | Role |
| --- | --- |
| `.magic/scripts/update-state.js` | Host of the progress recompute (§3, §5, §6), the scalar-field patch loop (§6.1), the line-cap guard incl. the unimplemented archival promise (§7, §10), the decision-section rebuild (§8), and `collectEntries()`, the entry-level grouping shared by both section rebuilds and the line-cap prune (§12) |
...
| `dev/tests/engine.js` | Regression harness carrying §11's and §12's cases, including the open §8, §9, and §10 obligations |
