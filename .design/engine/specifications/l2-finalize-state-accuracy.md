...
BAD : content.replace(/- \*\*Task:\*\* .*/, () => line)
      // rewrites the marker line; indented continuation lines survive as orphans
GOOD: content.replace(wholeEntryRe(/- \*\*Task:\*\* .*/), () => line)
      // the pattern gains (?:\r?\n[ \t]+\S.*)* — every indented, non-blank line
      // directly under the marker line is part of the entry it replaces
```

The continuation rule is deliberately **stricter** than `collectEntries()`'s. Inside an engine-owned list section (§8.3) nothing but entries can follow an entry, so "every following non-blank line" is safe there. Field lines are different: `**Phase:**` sits directly above `**Status:**` and the three `- **X:**` bullets are adjacent, with no blank line between them, so the same rule would swallow the *next field* — trading an orphan defect for a data-loss one. A continuation line is therefore one that is **indented and non-blank**, and an unindented line after a field is left exactly where it is. The cost is a known gap, accepted knowingly: a value wrapped *lazily* (continuation not indented — valid Markdown, but not the shape §12 or the reported files use, where every wrapped line is indented) is still not recognised, and its orphan persists as before. Recognising it would need a closed set of "starts a new entry" markers covering the bullet fields and the bold-label header fields alike, and an incomplete set fails silently by deleting a neighbour; leaving an orphan is the safer failure. Line breaks match as `\r?\n`, so a CRLF file is handled, and the last matched line's terminator is left in place, so line endings around the replacement are undisturbed. An indented sub-bullet under a field belongs to that field's entry and is replaced with it.

Regression coverage: `dev/tests/engine.js` gained three cases — a wrapped header field and all three wrapped `- **X:**` bullets replaced whole, with every neighbouring line and an unrelated wrapped decision left untouched (whole-file equality, modulo the timestamp); the strictness boundary (an unindented line after a field survives, an indented sub-bullet is consumed with its entry); and the same replacement in a CRLF file with no bare LF introduced; 94 → 97.
...
- `updateState()`'s scalar-field patch, given a field whose value is wrapped onto indented continuation lines, must replace the marker line **and** those continuation lines with the single new line, for a header field and for the `- **Task:**` / `- **Spec:**` / `- **Next Action:**` bullets alike. The assertion must be whole-file equality (modulo the volatile `**Updated:**` timestamp), not a presence check: only equality fails on a surviving orphan *and* on a patch that swallowed a neighbouring line. A companion case must pin the strictness boundary — an unindented line after a field survives untouched — and a CRLF fixture must be replaced whole without a bare LF appearing (§12.1).
