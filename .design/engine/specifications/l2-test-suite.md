...
**Finalize-pipeline coverage (mandatory):** because `scripts/finalize.js` is the single choke point for session-continuity (SC-2 of `l1-session-continuity.md`; SC-3 retired 2026-08-27), the harness MUST cover its non-trivial branches — any change to the finalize pipeline ships with a corresponding harness test:
...
- **Non-whitelisted file visibility (formerly SC-3.1)** — a `magic.run` fixture with one whitelist-matched change (`TASKS.md` status flip) plus one non-whitelisted change in the same working tree (e.g. a file under `dev/` or `.magic/`) must produce a stdout `### Changed artifacts` section naming **both** files, while significance/version-bump/CHANGELOG category still key off the whitelist-matched file alone. The prior fixture set only ever varied files inside the whitelist, so `emitSuccess()` reusing the whitelist-filtered set for the file *listing* — instead of the full working-tree diff — went unpinned; confirmed against a real commit already on `master` where 15 of 17 actually-changed files were invisible in finalize's own output. (The suggested-commit-message half of this requirement was retired 2026-08-27 alongside SC-3.)
