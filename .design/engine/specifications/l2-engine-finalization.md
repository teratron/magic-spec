...
- `Status` — **not patched by this step.** `updateSessionState()` calls `updateState(wsDir, { nextAction }, { autoProgress: true })` — `status` is never a key of that patch, so the field keeps whatever value an earlier explicit `--status=` call (`task.md` plan write-back, `run.md` phase transitions) last set. Left accurate only as long as those explicit call sites stay in sync — a latent gap tracked in the Known Gaps section of [l2-finalize-state-accuracy.md](l2-finalize-state-accuracy.md).
...
**Retired 2026-08-27** (explicit user directive — [l1-session-continuity.md](l1-session-continuity.md) §1.3). `commit-suggester.js` previously gained a fallback mode that emitted a suggested Conventional Commits message, labeled `(non-bumping)`, when the significance whitelist missed but the working tree had changed. That fallback, and the significant-path suggestion it complemented, are both removed: the pipeline no longer composes or prints a commit message on any path. Stdout-listing completeness (distinct from the retired message) is governed by [l2-finalize-output-contract.md](l2-finalize-output-contract.md) §3.

At the time of this retirement, the existing hard rule that no write-side git operation is ever invoked by the pipeline or the agent was noted as unaffected. That rule was itself retired later and independently — **2026-09-17** (explicit user directive — [l1-session-continuity.md](l1-session-continuity.md) §1.4): the pipeline takes no position on whether, when, or how the agent commits. `finalize.js` itself still performs read-only git probes only (`git diff`/`git log` for change detection) and still never calls a write-side git command — that remains a true fact about this script's own implementation, not a policy imposed on the agent calling it.
