...
The pre-commit hook (`install-hooks.js`) already worded this correctly and is unchanged. The trigger was a downstream agent's side note that `ENGINE_INTEGRITY` fired for `.magic/spec.md` and `.magic/task.md` (no version, no reproduction). The cause of *that* mismatch was not established: the latest release archive and every manifest since v2.1.87 match their own blobs, and the consumer installs available for inspection all pass their manifests with those two files byte-identical to upstream — so it is a local edit or a partial replacement, which the check cannot tell from a line-ending conversion. What this section fixes is the remedy path such a warning leads down.
...
| `.magic/scripts/utils.js` | Shared helpers — canonical path matcher (§Path Matching Contract); `hasEngineWriteTooling()`, the developer-repo / user-installation discriminator (§Engine Meta Update Flow) |
