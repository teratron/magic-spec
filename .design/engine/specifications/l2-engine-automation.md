...
**Concept-only spec extension** `[ADDED]`: a `Stable` L1 spec MAY be marked `concept-only` — a marker in the spec file, mirrored as a flag on its `PLAN.md` row — when its author judges no L2 implementation is currently planned. Concept authority: [l1-engine-core.md](l1-engine-core.md) §Known Process Gaps — Concept-Only Classification. `analyze-coverage.js`'s "Stable L1 without L2 child" coverage-gap advisory MUST exclude a spec carrying this marker from its gap count — the same `EXEMPT`-style exclusion as the bookkeeping-file set above, reported via its own `summary.conceptOnly` count rather than silently dropped. The marker is a *plan-intent* statement, not a permanent property: authoring any L2 spec with `Implements: {this L1}` MUST clear the marker on the next `check-prerequisites` pass, re-entering the L1 into normal coverage tracking. A rising `summary.conceptOnly` count is a narrated health signal in the Advisory Report (`/magic.analyze`), never suppressed.

```plaintext
BAD : coverage-gap advisory counts every Stable-L1-without-L2 the same way
      // a deliberate reference doc and a neglected gap both render as "gap"
GOOD: coverage-gap advisory excludes specs carrying concept-only
      summary.conceptOnly = count                 // reported, not hidden
      // clears automatically the moment a child L2 declares Implements:
```

**Required Fix** (Engine Improvement, out of this spec's write scope): `analyze-coverage.js`'s coverage-gap pass gains the `concept-only` check (marker read from the spec file or its `PLAN.md` row) before computing "Stable L1 without L2 child"; the write path that authors an L2's `Implements:` field clears the marker on its parent the moment the child is registered. `.magic/analyze.md`'s Advisory Report template gains a `concept-only` narration line for a non-zero, rising count.
