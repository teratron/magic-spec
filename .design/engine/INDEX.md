...
- **License**: MIT
- **Last Updated**: 2026-08-07 (No spec-count change, three amended, closing three ventilation findings: l2-engine-automation.md 1.7.0 → 1.8.0 — Coverage Denominator Scope EXEMPT set widened to include INDEX.md/RULES.md, evidenced by `.design/engine/INDEX.md` itself landing UNCOVERED this pass, superseding 1.7.0's "not evidenced as needed" call on those two files specifically; spec files, workspace.json, and active tasks/*.md remain not exempted. l1-workspace-intent-routing.md 1.1.0 → 1.1.1 — registered the previously-unlinked simulations subfolder matrix file as Canonical Reference `[SIM-MATRIX]`, resolving its UNCOVERED classification without relocating it. l2-spec-graph-memory.md 1.1.1 → 1.1.2 — normalized `Layer: 2` to `Layer: implementation`, matching all 15 other L2 specs (cosmetic, no logic change). Remaining ventilation advisory (non-blocking, not actioned here): l2-engine-diagnostics.md at 303 lines, 1% over the SPEC_BLOAT threshold.)
- **Previous**: 2026-08-07 (Spec count 30 → 32: **l2-engine-finalization.md decomposed** at 367 lines against the 300 `SPEC_BLOAT` threshold — its two accumulated defect registers extracted to **l2-finalize-state-accuracy.md** and **l2-finalize-output-contract.md**, parent slimmed to the pipeline contract and bumped to 2.0.0. Four backlog items closed in the same pass: R10 archival index rewrite (five-for-five reproduced, previously blocked on having no stated contract), R11 CHANGELOG entry suppression (closed bullet vocabulary × idempotent append, rotation helper imported but never called), R12 `addDecision` retrospec (shipped at 2.1.67 ahead of its spec), R13 plan-complete disambiguation (`l1-session-continuity.md` 1.8.0, new SC-2.4 — Pre-flight tests mechanical drift only, so a design-debt backlog cannot raise the HALT the funnel's own justification depends on).)
