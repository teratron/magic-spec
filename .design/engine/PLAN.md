...
*The **§8.4 example regex** item was removed 2026-08-07 — already resolved in l2-engine-finalization 1.10.1, gone stale in this list. **R10**, **R12**, and **R13** closed 2026-08-07 by Phase 19 (R10 confirmed live: the phase's own PLAN.md archival entry rewrote correctly, ending a run of five consecutive hand-corrections). **R11** is not fully closed: Phase 19's Track C root-caused the duplicate-heading anomaly (two 2026-05 manual edits predating the automated writer, not a live defect) but its own governing spec ([l2-finalize-output-contract.md](specifications/l2-finalize-output-contract.md) §4.4) bars a CHANGELOG fix from being planned until that diagnosis existed — it exists now, so the fix itself (rotation via `releaseUnreleased()`, and the still-open "what constitutes a release" design question §4.4 flags) is what remains, tracked below.*

- **R11 (remainder) — wire `[Unreleased]` rotation, decide what triggers it** ([l2-finalize-output-contract.md](specifications/l2-finalize-output-contract.md) §4.2, §4.4): `releaseUnreleased()` exists, is imported by `finalize.js`, and is never called — `[Unreleased]` has accumulated every entry since 2026-05-07 (192+ lines) with no rotation, which is the structural reason the closed bullet vocabulary exhausts itself instead of refreshing per release. Root-caused (Phase 19); the fix is barred no longer. The open design question §4.4 states but does not answer: what event constitutes a release in a project whose `.design/.version` patch-bumps on nearly every workflow invocation — answering it is a prerequisite to wiring the call, not a detail to leave implicit. Route via `/magic.spec`.
