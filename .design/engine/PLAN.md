...
**Based on:** .design/engine/INDEX.md v1.19.0
**Status:** Active
...
- **Debt-ceiling convention** ([l1-engine-core.md](specifications/l1-engine-core.md) v1.2.0 Known Process Gaps, pending ratification): a cap on open `Required Fix` blocks awaiting implementation, above which `/magic.spec`'s Pre-flight HALTs and recommends `/magic.task {ws}`. The spec deliberately mints no C-number — `.design/RULES.md` is the sole source of truth for the C-series and constitutional amendments are `/magic.rule`'s write scope. Path: `/magic.rule` to ratify, then `/magic.task` → `/magic.run` to build the Pre-flight gate in `.magic/spec.md`. **Update 2026-08-07**: the datapoint is in — Phases 15-20 (all now complete) closed the exact backlog that motivated the proposal through normal planning cycles, without a hard ceiling ever existing. Leaning toward "not needed," but minting a C-number is a constitutional-tier call reserved for explicit user ratification (Escalation Whitelist E4) — not decided here.
...
- **`DESIGN_DEBT_PENDING` gate has no parked/wontfix markup** (found 2026-08-07, this pass — this Backlog's first real encounter with the gate SC-2.4 shipped in Phase 19): `check-prerequisites.js` HALTs `magic.task` whenever `## Backlog` has any top-level bullet, with no way to distinguish "needs a spec" from "deliberately parked" (its own code comment says so explicitly — see `l2-engine-diagnostics.md`-style diagnostic `DESIGN_DEBT_GATE_NO_PARK_MARKUP`). Most of this list's items are the latter and will refire the gate every session regardless. Design question for `/magic.spec`: a markup convention (trailing tag, or a `## Deferred` subsection) the check can exclude from its count.
