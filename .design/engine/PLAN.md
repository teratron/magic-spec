...
- [ ] **STATE.md Accuracy & Generator Containment** ([l2-engine-finalization.md](specifications/l2-engine-finalization.md) v1.10.0 §7-§10) [L2] — implement the eight `Required Fix` blocks authored across seven field-report cycles and never executed: the SC-2 state-update step currently makes `STATE.md` *less* accurate than before the update it was supposed to refresh, and the finalize generator writes a spec's own identifier into the product's root `CHANGELOG.md`.
  ...
  - Tasks: [tasks/phase-15.md](tasks/phase-15.md)
