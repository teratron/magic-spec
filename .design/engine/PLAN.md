...
**Based on:** .design/engine/INDEX.md v1.15.16
**Status:** Active
...
- **Link sweep should exclude `.magic/templates/`** (ventilation, 2026-08-06): the Mode C link-integrity check reports 5 broken targets, every one a template placeholder — a default-workspace index path, two brace-delimited filename tokens, and a sibling-spec example — that resolves only once a template is instantiated into a user project. Same false-positive class as the code-span hits already excluded; a precision fix to a cognitive check, so it follows the R7 precedent of being spec'd first.
- **Registry cross-reference reads inside code spans** (found 2026-08-06, immediately after the entry above was written): `check-prerequisites` extracts spec references from PLAN.md prose without stripping code spans, so quoting a placeholder filename in the Backlog raised a `REGISTRY_MISMATCH` for a spec that does not and should not exist. Third instance of one root pattern — the archiver eligibility predicate, the link sweep, and now the registry cross-reference all match markdown that documents a token rather than uses it. Candidate for a shared strip-then-scan helper rather than a third local fix.
