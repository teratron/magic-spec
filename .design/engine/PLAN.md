...
- [ ] **Engine Diagnostics Digest** ([l1-engine-diagnostics.md](specifications/l1-engine-diagnostics.md)) [L1] — record non-fatal findings instead of only printing them, accumulate across the separate processes of one invocation, and deliver them once as a deduplicated, severity-ordered digest immediately before the next step.
- [ ] **Diagnostics Implementation** ([l2-engine-diagnostics.md](specifications/l2-engine-diagnostics.md)) [L2] — `lib/diagnostics.js` collector, JSONL sink under the already-gitignored `.design/.cache/`, `record-diagnostic` executor subcommand, single `emitTail()` in `finalize.js`, and migration of all 17 non-fatal emitters across 6 scripts.
  ...
  - Tasks: [tasks/phase-18.md](tasks/phase-18.md)

## Backlog

- **R11 — CHANGELOG dedup suppresses genuine entries because the bullets are too generic** (found 2026-08-07, both finalize invocations of the Phase 18 planning cycle reported `CHANGELOG | skipped (duplicate)`): `buildChangelogBullet()` composes workflow-scoped generic text (`Updated 5 specifications (engine)`, `Updated task plan and task index (engine)`), and `appendBullet()` correctly refuses to add a byte-identical duplicate. Both behaviors are individually right; together they mean that once a shape has appeared in `[Unreleased]`, every later cycle producing the same shape writes **nothing**, and the root CHANGELOG silently under-reports user-facing work. Verified against the live file: both of this cycle's bullets already existed from prior cycles. Note the tension with RC-11, which deliberately stripped artifact IDs from these bullets to stop the generator leaking spec identifiers into a product file — genericness is not an accident, it is the containment fix, so the resolution cannot simply be "put the names back". Needs a stated contract before it can be built (the R7/R10 precedent): what distinguishes two cycles' worth of "updated some specs" in release notes, given the identifiers that would distinguish them are exactly what RC-11 forbids. Route via `/magic.spec`.
- **R10 — phase archiver rewrites the link target but not its label** (found 2026-08-06, reproduced on every archival since — Phases 14, 15, 16, and now 17, four for four): `updatePlanIndex()` in `phase-archiver.js` replaces `(tasks/phase-N.md)` with `(archives/tasks/phase-N.md)` and leaves the `[tasks/phase-N.md]` label untouched, yielding a working link whose text contradicts its destination. Cosmetic, not a broken link, so it never surfaces in a link sweep — which is why it survived. `updateTasksIndex()` is unaffected (its label is a phase number, not a path). Blocked on spec coverage, not on effort: [l2-engine-finalization.md](specifications/l2-engine-finalization.md) §2 describes `phase-archiver.js` as rewriting *`TASKS.md`* link references and does not describe `updatePlanIndex()` at all, so there is no stated contract to implement against — same shape as R7, spec'd in §6 before it was fixed. Four confirmed occurrences is enough evidence to stop deferring: the next `/magic.spec` pass should take this alongside whatever else is queued, rather than waiting for a fifth.
